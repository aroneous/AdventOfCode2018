...
processCart :: Grid -> Carts -> (Int, Int) -> Cart -> (Carts, Maybe (Int, Int))
processCart grid carts (y, x) cart =
    let (y', x') = newCoord (y, x) (direction cart)
        ...
        carts'' = Map.insert (y', x') (Cart newDir newTurn) carts'
    in if collision
        then (carts, Just (y', x'))
        else (carts'', Nothing)
...
iterateGrid :: Grid -> Carts -> (Carts, Maybe (Int, Int))
iterateGrid grid carts = Map.foldlWithKey (\(carts', maybeCollision) coord cart ->
    case maybeCollision of
        Nothing -> processCart grid carts' coord cart
        Just collisionCoord -> (carts', maybeCollision)) (Map.empty, Nothing) carts

findCollision :: Grid -> Carts -> (Int, Int)
findCollision grid carts =
    let (carts', maybeCollision) = iterateGrid grid carts
    in case maybeCollision of
        Just boom -> boom
        Nothing -> findCollision grid carts'
