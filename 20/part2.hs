...
count1kOrFarther :: Doors -> Int -> Set Coord -> Set Coord -> Int
count1kOrFarther doors iter seen starts
    | Set.null starts = 0
    | otherwise = 
        let freshMeat = findUnseenNeighbors doors seen starts
            seen' = Set.union seen freshMeat
            count = if iter >= 1000 then Set.size freshMeat else 0
        in count + (count1kOrFarther doors (iter + 1) seen' freshMeat)

main = do
    contents <- getContents
    let (patt, _) = parsePattern $ tail contents
        (ends, doors) = iteratePattern (Set.singleton (0,0)) patt
        numFar = count1kOrFarther doors 1 (Set.singleton (0,0)) (Set.singleton (0,0))
    --print doors
