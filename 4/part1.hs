...
main = do
    ...
    let l = lines contents
        ...
        bySleepiness = List.sortOn (totalSleepTime.snd) (Map.toList sleepTimes)
        sleepiest = last bySleepiness
        minuteMap = makeMinuteMap (snd sleepiest)
        sleepiestMinute = List.maximumBy (Data.Ord.comparing snd) (reverse (Map.toList minuteMap))
--    print (map (\(id, mm) -> (id, totalSleepTime mm)) bySleepiness)
    print minuteMap
    print sleepiestMinute
    print ((fst sleepiest) * (fst sleepiestMinute))
